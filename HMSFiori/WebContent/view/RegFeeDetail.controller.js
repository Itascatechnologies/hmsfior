sap.ui.controller("test.my.fiori.view.RegFeeDetail", {
	...
	onInit : function() {
		...
		if (sap.ui.Device.system.phone) {
			...
			oEventBus.subscribe("RegFeeMaster", "InitialLoadFinished",
	...
	onRouteMatched : function(oEvent) {
		...
		jQuery.when(this.oInitialLoadFinishedDeferred).then(
				jQuery.proxy(function() {
					...
					if (oParameters.name !== "RegFeeDetail") {
						return;
					}
					...
					this.getEventBus().publish("RegFeeDetail", "TabChanged", {
						sTabKey : sTabKey
					});
	...
	_showFormFragment : function(fType) {
		var oPage = this.getView().byId("idDetailPage");
		var sFragmentName = "RegFee_Read";
		if (fType == "Edit") {
			sFragmentName = "RegFee_Edit";
		}
		oPage.removeAllContent();
